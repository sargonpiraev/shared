...
| `project__lefthook.yml.yaml`            | clone-root `lefthook.yml` must `extends` `lefthook-config/lefthook.yml` and must not set git `remotes:` (extends-only; meta lefthook.yml overlay is not this leaf)                                                        |
